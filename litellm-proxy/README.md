...
**Important:** That backend string must be a **real Ollama model name**—exactly what appears in `ollama list` or `GET /api/tags` (e.g. `ollama/qwen2.5-coder:32b`, `ollama/gemma4:31b`). The default **`gpt-4o-mini`** route uses **`ollama/gemma4-cursor:31b`**: same weights as `gemma4:31b` but a **Modelfile** system prompt that discourages fake JSON / ReAct output from Cursor-style prompts (create it with **`ollama create gemma4-cursor:31b -f ../modelfiles/gemma4-cursor-31b`** from the repo root, or run **`./scripts/create-aliases.sh`**). Hyphenated **aliases** you create for Cursor’s UI are optional and only help when talking **directly** to Ollama’s OpenAI API; LiteLLM still passes through to Ollama, so if an alias is wrong or missing on the LLM host, Ollama returns “invalid model” and Cursor only repeats that upstream error. Match the tag on the LLM host and restart the proxy after edits.
